...
for server in servers:
	...
	if not args.test_mode and bool(serverProcessed):
		for id in serverProcessed:
			if id['key'] is server['key'] and id['price'] is server['price']:
				continue
	...
	disk_descriptors=[desc for desc in server['information'] if " GB" in desc or " TB" in desc] # maybe fixed?
